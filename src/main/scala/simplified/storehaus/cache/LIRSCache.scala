 ...
 * download;jsessionid=EA23F554FDF98A258C6FDF0C8E98BFD1?doi=10.1.1.116.2184&rep=rep1&type=pdf
 *
...
object LIRSCache {
  def apply[K, V](
    maxSize: Int,
    sPercent: Double,
    backingMap: Map[K, V] = Map.empty[K, V]
  ): LIRSCache[K, V] = {
...
class LIRSCache[K, V](lirsStacks: LIRSStacks[K], backingMap: Map[K, V]) extends Cache[K, V] {
  ...
  def put(kv: (K, V)): (Option[K], Cache[K, V]) = {
    val (k, v) = kv
    def miss: (Option[K], Cache[K, V]) = {
      if (!lirsStacks.isSFull) {
        ...
        older match {
          case None => (None, new LIRSCache(newLirsStacks, backingMap + kv))
          case _ => throw new IllegalStateException(
            "Stack was not full, yet evicted when element was added")
        }
      } else {
        val (evictedFromS, newLirsStacks1) = lirsStacks.putOnTopOfS(k)
        evictedFromS match {
          case Some(oldK) =>
            // We know that S is full, and that k is not in S. We add our thing to Q, and take
            // out of the cache anything that comes out.
            // We then add it to S, and if we get something out, take it out of the cache if it
            // is also not in Q. We add kv to the cache.
            val (evictedFromQ, newLirsStacks2) = newLirsStacks1.putOnTopOfQ(k)
            val (evicted1, newBackingMap1) = evictedFromQ match {
              case Some(oldk) => (Some(oldk), backingMap - oldk)
              case None => (None, backingMap)
            }
            val (evicted2, newBackingMap2) =
              if (newLirsStacks2.isInQ(oldK)) {
                (evicted1, newBackingMap1)
              } else {
                (Some(oldK), newBackingMap1 - oldK)
              }
            (evicted2, new LIRSCache(newLirsStacks2, newBackingMap2 + kv))
          case None =>
            // We know that S is full, and that k is in S. This is a non-resident HIR block. We
            // have bumped k in S and got nothing back. Then we take the oldest element of S and
            // add it to Q. If we get something back, we take it out of the cache. Then we prune.
            ...
            oldestInS match {
              case Some(oldKinS) =>
                ...
                val (evicted, newBackingMap1) = evictedFromQ match {
                  case Some(oldKinQ) => (Some(oldKinQ), backingMap - oldKinQ)
                  case None => (None, backingMap)
                }
                (evicted, new LIRSCache(newLirsStacks3.prune, newBackingMap1))
              case None => (None, new LIRSCache(newLirsStacks2.prune, backingMap))
            }
        }
      }
    }
    get(k) match {
      case Some(oldV) => if (oldV != v) miss else (None, this)
      case None => miss
    }
  }
  ...
  def hit(k: K): Cache[K, V] =
    get(k).map { v =>
      if (lirsStacks.isInS(k) && !lirsStacks.isInQ(k)) {
        // In the case where k is in S but not in Q, it is a LIR block. We push it to the top of S,
        // and prune if it was the oldest in S.
        val (evictedFromS, newLirsStacks) = lirsStacks.putOnTopOfS(k)
        if (evictedFromS.isDefined) {
          throw new IllegalStateException(
            "Nothing should have been evicted from S when k was bumped as it was already present")
        }
        new LIRSCache(if (lirsStacks.isOldestInS(k)) newLirsStacks.prune else newLirsStacks,
          backingMap)
      } else if (lirsStacks.isInS(k) && lirsStacks.isInQ(k)) {
        // In the case where k is in S and Q, it is an HIR block. We bump k to the top of S and
        // remove it from Q. We then move the oldest value in S to Q. Then we prune.
        val (evictedFromS, newLirsStacks) = lirsStacks.putOnTopOfS(k)
        if (evictedFromS.isDefined) {
          throw new IllegalStateException(
            "Nothing should have been evicted from S when k was bumped as it was already present")
        ...
        oldestInS match {
          case Some(oldK) =>
            ...
            if (evictedFromQ2.isDefined) {
              throw new IllegalStateException(
                "Nothing should have been evicted when we put the value from S on top of Q")
            }
            new LIRSCache(newLirsStacks4.prune, backingMap)
          case None => throw new IllegalStateException(
            "We dropped the oldest value in S but got nothing back")
        }
      } else if (!lirsStacks.isInS(k) && lirsStacks.isInQ(k)) {
        // In the case where k is not in S but it is in Q it is a non-resident HIR block. We bump
        // it to the top of Q, and put it in S. If anything is evicted from S in the process, we
        // check if it is in Q. If it is not, we remove it from the cache.
        val (evictedFromQ, newLirsStacks) = lirsStacks.putOnTopOfQ(k)
        if (evictedFromQ.isDefined) {
          throw new IllegalStateException(
            "We bumped the value to the top of Q, so nothing should have come back")
        }
...
object LIRSStacks {
  def apply[K](sSize: Int, qSize: Int): LIRSStacks[K] =
    new LIRSStacks[K](Stack[K](sSize), Stack[K](qSize))
}
...
class LIRSStacks[K](stackS: Stack[K], stackQ: Stack[K]) {
  ...
  final def prune: LIRSStacks[K] = {
    ...
    oldK match {
      ...
      case _ => this
      // We don't need to remove as there is either nothing to remove, or an LIR
  ...
  def empty: LIRSStacks[K] = new LIRSStacks(stackS.empty, stackQ.empty)

  override def toString: String = s"S:[$stackS] Q:[$stackQ]"
}
...
object Stack {
  def apply[K](
    ...
    cyclicIncrementProvider: IdProvider[CyclicIncrement[Int]] =
      CyclicIncrementProvider.intIncrementer
  ): Stack[K] = new Stack[K](maxSize, backingIndexMap, backingKeyMap, cyclicIncrementProvider)
}
...
class Stack[K](
  ...
  def dropOldest: (Option[K], Stack[K]) = {
    if (isEmpty) {
      ...
      (Some(lastK), new Stack(maxSize, backingIndexMap - lastInc, backingKeyMap - lastK,
        cyclicIncrementProvider.cull(lastInc)))
    }
  }
  ...
  def remove(k: K): (Option[K], Stack[K]) = {
    backingKeyMap.get(k) match {
      case Some(inc) => (Some(k), new Stack(maxSize, backingIndexMap - inc, backingKeyMap - k,
        cyclicIncrementProvider.cull(inc)))
      case None => (None, this)
    }
  }
  ...
  def size: Int = backingIndexMap.size
  def isFull: Boolean = size >= maxSize
  def isEmpty: Boolean = size <= 0
  def empty: Stack[K] = new Stack[K](maxSize, backingIndexMap.empty, backingKeyMap.empty,
    cyclicIncrementProvider.empty)
  override def toString: String = backingIndexMap.values.mkString(",")
}
