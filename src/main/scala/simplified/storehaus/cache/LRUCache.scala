...
class LRUCache[K, V](maxSize: Long, idx: Long, map: Map[K, (Long, V)], ord: SortedMap[Long, K])
  ...
  override def put(kv: (K, V)): (Option[K], LRUCache[K, V]) = {
    val (key, value) = kv
    val newIdx = idx + 1
    val (evictedKey, newMap, newOrd) =
      if (ord.size >= maxSize) {
        val (idxToEvict, keyToEvict) =
          map.get(key).map { case (i, _) => (i, key) }
            .getOrElse(ord.min)
        (Some(keyToEvict), map - keyToEvict, ord - idxToEvict)
      } else {
        (None, map, ord)
      }

    (evictedKey, new LRUCache(maxSize, newIdx,
